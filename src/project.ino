...
int pH_Value; 
float Voltage;
float calibration_value = 21.34 + 0.67;
int phval = 0; 
...
void sendSensor()             // to read the readings from the sensor and send the data to the Blynk app
{
...
{   
  ...
  pinMode(pH_Value, INPUT);       // For pH sensor
...
void readpH() {
  for(int i=0;i<10;i++) 
 { 
 buffer_arr[i]=analogRead(A0);      //GPIO 36
 delay(30);
 }
 for(int i=0;i<9;i++)
 {
...
void displaySensors() {
  ...
  if (ph_act < 7) {
    ...
    display.print("Water is ACIDIC");
  } else if (ph_act = 7) {
    display.setCursor(0, 50);
    display.print("Water is NEUTRAL");
  } else  {
    display.setCursor(0, 50);
    display.print("Water is BASIC");
  }
